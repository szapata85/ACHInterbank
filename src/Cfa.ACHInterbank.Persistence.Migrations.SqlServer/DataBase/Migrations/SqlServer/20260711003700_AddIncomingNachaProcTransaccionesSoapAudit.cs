    ...
    public partial class AddIncomingNachaProcTransaccionesSoapAudit : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql("""
                ...
                SET
                    [SoapMethodName] = CASE WHEN [MethodName] IS NULL OR LTRIM(RTRIM([MethodName])) = '' THEN [SoapMethodName] ELSE [MethodName] END,
                    [SoapResponseCode] = CASE WHEN [ResponseCode] IS NULL OR LTRIM(RTRIM([ResponseCode])) = '' THEN [SoapResponseCode] ELSE [ResponseCode] END,
                    [SoapResponseDescription] = CASE WHEN [ResponseMessage] IS NULL OR LTRIM(RTRIM([ResponseMessage])) = '' THEN [SoapResponseDescription] ELSE [ResponseMessage] END,
                    [IsSuccessful] = [IsSuccess],
                    [SoapTechnicalStatus] = CASE
                        WHEN [IsSuccess] = CAST(1 AS bit) THEN 'Succeeded'
                        WHEN [IsRetryable] = CAST(1 AS bit) THEN 'RetryableFailure'
                        ELSE 'UnknownFailure'
                    END
                """);
