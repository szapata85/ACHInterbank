...
public class DynamicJobExecutor
{
    ...
    public DynamicJobExecutor(IServiceProvider sp, ILogger<DynamicJobExecutor> logger, IEnumerable<ITaskHandler> handlers, QuartzTaskCalendarEvaluator calendarEvaluator)
    {
        _sp = sp;
        _logger = logger;
        _handlers = handlers;
    ...
    public async Task ExecuteAsync(IJobExecutionContext context)
    {
        ...
        var task = await db.TaskDefinitions.Include(t => t.Parameters).FirstOrDefaultAsync(t => t.Id == taskId, context.CancellationToken);
        if (task is null)
        {
    ...
    private static async Task<(bool Success, string? Output, string? Error)> ExecuteHandlerWithRetryAsync(ITaskHandler handler, TaskDefinition task, CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (backoff > TimeSpan.Zero) await Task.Delay(backoff, cancellationToken);
            }
        }

        return (false, null, $"Reintentos agotados tras {maxAttempts} intento(s). Última excepción: {lastException}");
    }
}
