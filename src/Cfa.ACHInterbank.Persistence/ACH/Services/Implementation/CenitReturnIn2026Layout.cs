...
/// <summary>
/// Contrato físico ejecutable para devoluciones entrantes CENIT.
/// Fuente: Manual de Especificaciones Formato NACHA-M CENIT, 07-may-2026,
/// secciones 7.2.1, 7.3.1 y Anexo 1.6.
/// </summary>
internal static class CenitReturnIn2026Layout
{
    public const int RecordLength = 106;
    public const string ProfileCode = "OFFICIAL_CENIT_ENTRADA_DEVOLUCION_V2026_1_0";
    public const string CtxScopeStatus = "CTX_OUT_OF_CURRENT_PLATFORM_SCOPE";
    public const string VariantPrefix = "CENIT_RETURN_IN_2026_R";
    public const string Addenda99Variant = "CENIT_RETURN_IN_2026_R7_ADDENDA_99";

    private const string Source = "Manual de Especificaciones Formato NACHA-M CENIT";
    private const string Version = "2026-05-07";
    private const string Section = "7.2.1; 7.3.1; Anexo 1.6";

    private static readonly IReadOnlyCollection<string> OrdinaryReturnCauses = Array.AsReadOnly(
    [
        "R01", "R02", "R03", "R04", "R06", "R07", "R08", "R09", "R10", "R12",
        "R13", "R14", "R15", "R16", "R17", "R20", "R23", "R29", "R31", "R32",
        "R33", "R34", "R35"
    ]);

    private static readonly HashSet<string> OrdinaryReturnCauseSet =
        new(OrdinaryReturnCauses, StringComparer.OrdinalIgnoreCase);

    private static readonly HashSet<string> ReturnOfReturnCauseSet =
        new(Enumerable.Range(60, 15).Select(value => $"R{value}"), StringComparer.OrdinalIgnoreCase);
        ...
        new ReadOnlyDictionary<string, IReadOnlyList<AchColOfficialFieldDescriptor>>(
            new Dictionary<string, IReadOnlyList<AchColOfficialFieldDescriptor>>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["7"] = Fields(
                    ...
                    F("T7-RETURN-REASON", "7", "RETURNREASONCODE", 4, 3, NachaFieldDataType.Alphanumeric, true, 'L', ' ', allowed: OrdinaryReturnCauses),
    ...
    internal static bool IsProfile(string? profileCode) => string.Equals(profileCode, ProfileCode, StringComparison.Ordinal);
    internal static bool IsOrdinaryReturnCause(string? cause) => !string.IsNullOrWhiteSpace(cause) && OrdinaryReturnCauseSet.Contains(cause.Trim());
    internal static bool IsReturnOfReturnCause(string? cause) => !string.IsNullOrWhiteSpace(cause) && ReturnOfReturnCauseSet.Contains(cause.Trim());
    ...
    internal static bool TryParseReturnAddenda(string record, out CenitReturnAddenda2026? addenda)
    {
        addenda = null;
        if (record.Length != RecordLength || record[0] != '7' || record.Substring(1, 2) != "99")
        {
            return false;
        }
        ...
        var sequence = record.Substring(81, 15).Trim();
        if ((reason.Length != 3 || reason[0] != 'R' || !reason.AsSpan(1).ToString().All(char.IsDigit))
            || originalTrace.Length != 15 || !originalTrace.All(char.IsDigit)
            || sequence.Length != 15 || !sequence.All(char.IsDigit))
        {
            return false;
        }

        addenda = new CenitReturnAddenda2026(reason, originalTrace, sequence, record.Substring(29, 8).Trim(), record.Substring(37, 44).TrimEnd());
        return true;
    }
    ...
    private static AchColOfficialFieldDescriptor F(
        ...
        new($"CENIT-2026-RETURN-IN-{suffix}", recordCode, fieldCode, start, length, dataType, required,
            justification, pad, format, allowed, NormativeSection: Section, NormativeSource: Source, NormativeVersion: Version);
}

internal sealed record CenitReturnAddenda2026(
