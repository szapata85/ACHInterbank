...
public class TransactionsController : ControllerBase
{
    ...
    [EndpointDescription("Qué consulta: evalúa reglas de política para una transacción candidata sin persistirla. Quién lo usa: canales de integración, soporte y operación para validar viabilidad previa. Permiso requerido: controller sin [Authorize] explícito; aplica control global configurado. Tipo: consulta de simulación. Impacto operacional: reduce rechazos posteriores al anticipar reglas de prioridad, duplicidad y restricciones. Auditoría/trazabilidad: debe conservarse evidencia de solicitud y resultado de preview para diagnósticos. Riesgos: preview con datos incompletos puede generar falsa sensación de aprobación. Errores esperados: 400 solicitud inválida; 401/403 seguridad global; 500 no controlado. Relación ACH/transacciones: se centra en validación previa de entidad transacción; no procesa archivos ni lotes como BulkIngestionController.")]
    ...
    [EndpointDescription("Qué consulta: recupera una transacción específica para soporte, conciliación o auditoría técnica. Quién lo usa: operación ACH, soporte de incidentes y auditoría. Permiso requerido: no se declara [Authorize] en el controller; aplica seguridad global. Tipo: consulta. Impacto operacional: habilita análisis de caso puntual sin modificar estado. Auditoría/trazabilidad: debe registrarse quién consultó, id solicitado y contexto de diagnóstico. Riesgos: consultar id incorrecto puede conducir a análisis equivocado. Errores esperados: 401/403 seguridad global; 404 transacción no encontrada; 500 no controlado. Relación con BulkIngestionController: consulta entidad transacción, no estado de batch de ingestión moderna.")]
    [HttpGet("{id:int}")]
