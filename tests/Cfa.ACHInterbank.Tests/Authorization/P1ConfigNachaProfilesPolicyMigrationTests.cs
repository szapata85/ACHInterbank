...
public class P1ConfigNachaProfilesPolicyMigrationTests
{
    [Fact]
    public void NachaConfigProfilesController_ComposicionAuthorizeCorrecta()
    {
        var controllerAttrs = typeof(NachaConfigProfilesController).GetCustomAttributes<AuthorizeAttribute>(true).ToList();
        Assert.Contains(controllerAttrs, a => string.IsNullOrWhiteSpace(a.Policy));

        var methods = typeof(NachaConfigProfilesController).GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
