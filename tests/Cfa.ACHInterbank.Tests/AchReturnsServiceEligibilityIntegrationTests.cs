...
public class AchReturnsServiceEligibilityIntegrationTests
{
    ...
    public async Task AchReturnsService_ShouldUseEligibilityService_BeforeGeneratingReturn()
    {
        ...
        context.ClearingHouses.Add(new ClearingHouse { Id = 7002, Name = "ACH Colombia", Code = "ACH", OriginCode = "000101006" });
        context.AchCycles.Add(new AchCycle { Id = "C1", CycleName = "C1", ProcessingDate = DateTime.UtcNow.Date, CutoffTime = new TimeSpan(8, 0, 0), ClearingHouseId = 7002 });
        context.AchTransactions.Add(new AchTransaction { Id = 1, AchCycleId = "C1", Type = TransactionTypeEnum.Debit, State = AchTransferStateEnum.Pending, EffectiveEntryDate = DateTime.UtcNow.Date, TransactionCode = "27", TraceNumber = "123456789012345", ReceivingDFI = "12345678", OriginatingDFI = "12345678", Amount = 100m, Reference = "R", SourceAccountNumber = "1", DestinationAccountNumber = "2" });
