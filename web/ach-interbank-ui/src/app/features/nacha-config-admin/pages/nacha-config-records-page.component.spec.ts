...
describe('NachaConfigRecordsPageComponent', () => {
  ...
  it('Component_ShouldBlockDuplicateSequencesAndCancelWithoutPersisting', () => {
    component.onSequenceChange(component.records[0], { target: { value: '20' } } as unknown as Event);

    expect(component.sequenceError).toContain('secuencia diferente');
    component.guardarSecuencia();
    expect(commandSpy.actualizarSecuencia).not.toHaveBeenCalled();

    component.cancelarCambios();
    expect(component.records[0].sequence).toBe(10);
    expect(component.sequencesDirty).toBeFalse();
    expect(component.sequenceError).toBe('');
  });
});
