...
TXCODE_CUTOVER_BASELINE: CLOSED — The first immutable TXCODE-aware publication cohort establishes ACH 35.1 under NormativeVersion V35 and fixed CENIT 1.2 under unchanged 2026-05-07 normative metadata. Supported CENIT 1.0/1.1 predecessor history and all legacy publications/snapshots remain untouched. Snapshot parity with the characterized static ordinary matrix is required at bootstrap, chamber-scoped code non-reassignment begins with this cohort, and no schema or migration was introduced. TransactionPersister static authority remains temporarily by design.
CURRENT_DELTA: NACHA-RULE-METADATA remains PARTIAL. TXCODE-ORDINARY is not closed; the next slice is the runtime TXCODE authority cutover. Static layout authority removal, explicit official-profile version workflow, direct/global DbContext immutability hardening, and unrelated CTX debt remain open.
