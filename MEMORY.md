...
REMAINING_STATIC_LAYOUT_AUTHORITY: Supported ordinary inbound preselection and outbound static layout authority are CLOSED. Inbound postselection CENIT parser static profile predicates remain outside this preselection slice; review separately if pursuing global inbound static-layout removal.
...
GAP-CENIT-PUBLISHED-CARDINALITY-001: OPEN — Published CENIT cardinality policy correction remains pending. Inbound postselection static-authority removal remains BLOCKED until that policy correction is certified. NACHA-RULE-METADATA remains PARTIAL.
CURRENT_DELTA: NACHA-RULE-METADATA remains PARTIAL. Runtime ordinary TransactionCode authority, supported ordinary outbound static layout authority, and ordinary inbound preselection authority are CLOSED. Inbound postselection static parser validation, explicit official-profile version workflow, direct/global DbContext immutability hardening, and unrelated CTX debt remain open.
