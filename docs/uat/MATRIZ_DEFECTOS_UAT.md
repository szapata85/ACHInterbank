...
| DEF-UAT-SOAP-MAP-001 | UAT-SOAP-MAP | Media | `Proc_Contrapartidas` puede generar payload con fallback transicional si no existe `IntegrationMappingSet` publicado. | Integracion SOAP / mappings | `docs/uat/evidencias/soap-integrations/mapping-trace/proc_contrapartidas/mapping_trace.md` | Arquitectura/Integracion | Abierto | 2026-05-21 | PENDIENTE | Eliminar/gobernar fallback o exigir mapping publicado con trace formal antes de cierre normativo. |
| DEF-UAT-SOAP-MAP-002 | UAT-SOAP-MAP | Alta | `Proc_Transacciones` usa mapping publicado para payload, pero no se observo guardrail DryRun/Disabled especifico equivalente a `Proc_Contrapartidas`. | Integracion SOAP / UAT guardrail | `docs/uat/evidencias/soap-integrations/mapping-trace/proc_transacciones/validation_report.md` | Arquitectura/Integracion/Seguridad | Abierto | 2026-05-21 | PENDIENTE | Agregar modo UAT/local no transmisivo antes de ejecutar flujo externo real. |
