...
| DEF-UAT-007 | UAT-REAL-025 | Media | El stack local no requiere servicio externo de secretos. | Custodia de secretos | `docker-compose.yml` | Seguridad/Operaciones | Cerrado | 2026-06-12 | CERRADO | La custodia real usa el mecanismo corporativo aprobado fuera del compose. |
...
| DEF-UAT-009 | UAT-REAL-032 | Media | Health checks actuales cubren live/ready DB, pero no evidencian Quartz ni dependencias externas. | Observabilidad | `src/Cfa.ACHInterbank.Api/DependencyInjectionService.cs` | Tecnologia/Operaciones | Abierto | 2026-05-18 | PENDIENTE | Ampliar controles o documentar alcance. |
...
- `WSCFAACH / Proc_Contrapartidas / MonetaryDebitRequest / OutboundRequest` visible y seleccionable en SPA.
...
- Evidencia visual/DOM: `docs/ux/evidencias/integration-mappings-ux-validation.json`, `docs/ux/evidencias/integration-mappings-proc-contrapartidas-validation.json`.
