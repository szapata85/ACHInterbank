...
Estado: matriz actualizada tras estabilizacion final UAT/readiness.
...
| DEF-UAT-015 | UAT-TECH-006 | Media | Rol esperado `ACH.Operator` no aparece en respuesta de login ni JWT; rol visible `Admin` autoriza menu y endpoints read-only por permisos. | Identidad/Autorizacion | `RoleConfiguration` contiene `ACH.Operator`; `UserRoleConfiguration` asigna `admin` solo a `Admin`; login sanitizado muestra roles respuesta/JWT = `Admin` | Seguridad/Tecnologia | Abierto por seed/seguridad | 2026-05-18 | PENDIENTE | Definir si `admin` demo debe recibir tambien `ACH.Operator` via migracion/seed controlado o si `Admin` cubre UAT tecnico. No se modifico auth ni BD en esta fase. |
...
| DEF-UAT-020 | UAT-REAL-006 | Alta/Media | Validacion NACHA-M campo-a-campo y homologacion externa de registros 1/5/6/7/8/9 sigue parcial, aunque existen layouts, builders, validators y tests tecnicos. | NACHA-M layouts/normativa | `docs/go-live-readiness/MATRIZ_NACHA_M_LAYOUTS.md`, `docs/uat/nacha-records-acceptance-checklist.md` | Compliance/Operaciones/Tecnologia | Abierto | 2026-05-19 | PENDIENTE | Ejecutar comparativo formal contra normativa/camara, generar archivo controlado sin datos reales, validar hash/totales/block count y obtener firma o waiver. |
