...
| Scorecard | 68.1 / 100 | docs/go-live-readiness/SCORECARD_GO_LIVE_READINESS.md |
...
- DEF-UAT-021: cerrado tecnicamente; `/NachaExport/{cycleId}` devuelve 422 JSON controlado si faltan prerequisitos, no archivo 0 bytes como exito.
- DEF-UAT-022: cerrado tecnicamente para UAT/local; `Proc_Contrapartidas` usa `DryRun` por defecto y no transmite externamente sin modo `Live` explicito.
