...
Estado: plan de trabajo actualizado tras estabilizacion final UAT/readiness; incluye correcciones de bajo riesgo aplicadas y pendientes de validacion.
...
| G-28 | Cerrar validacion formal NACHA-M 1/5/6/7/8/9 campo-a-campo. | Normativa/UAT/Evidencia | Alto | `docs/go-live-readiness/MATRIZ_NACHA_M_LAYOUTS.md`, evidencias externas | Compliance/Operaciones/Tecnologia | Normativa/camara y datos anonimizados | Archivo controlado sin datos reales, hash/totales/block count validados y matriz firmada o waiver | Pendiente |
| G-29 | Definir asignacion de rol `ACH.Operator` para UAT. | Seguridad/Seed/Auth | Medio | `UserRoleConfiguration` o migracion/seed controlado posterior; docs seguridad | Seguridad/Tecnologia/QA | Decision matriz rol-permiso | JWT/respuesta evidencia `ACH.Operator` con usuario sintetico aprobado o decision formal de usar `Admin` para UAT tecnico | Pendiente decision |
