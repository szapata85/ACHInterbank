...
Estado: plan actualizado tras cierres tecnicos G3.5-G3.6; Productivo permanece NO-GO.
...
| G-04 | Cerrar naming externo por camara/flujo. | Validacion negocio/proveedor | Alto | Docs naming y homologacion | Compliance/Operaciones | Normativa/camara | Tecnico cerrado; matriz firmada externa pendiente | Parcial normativo |
...
| G-11 | Definir custodia de secretos para UAT o excepcion. | Configuracion/Seguridad | Medio | compose UAT/documentacion/scripts | Seguridad/Operaciones | Decision ambiente | Mecanismo validado o waiver | Parcial - requiere decision |
...
| G-15 | Completar monitoreo productivo. | Operacion | Medio | Docs monitoreo/alertas | Tecnologia/Operaciones | Alcance observabilidad | Quartz UAT evidenciado; monitoreo productivo aprobado | Parcial |
...
| G-28 | Cerrar validacion formal NACHA-M 1/5/6/7/8/9 campo-a-campo. | Normativa/UAT/Evidencia | Alto | `docs/uat/UAT_NACHA_M_CAMPO_A_CAMPO.md`, matrices por camara | Compliance/Operaciones/Tecnologia | Tecnico G3.5/G3.6 cerrado | Homologacion externa, matriz firmada o waiver | Parcial normativo |
...
| G-32 | Preparar prenotificacion UAT valida para NACHA-M no vacio. | UAT/Operaciones | Alto | Flujo G3.6B | Operaciones/QA/Arquitectura ACH | Datos sinteticos y ciclo existente | Prenotificacion controlada, archivo `.6` no vacio y regla de tres dias validada | Cerrado tecnico UAT |
...
Estado especifico UAT funcional sintetico: G-24 queda parcialmente cubierto por API directa y reintento HTTP desde SPA Docker. G-25 queda corregida tecnicamente. G-26 queda cerrada funcionalmente para nuevas transacciones tras revalidacion con `UAT-SINT-TRACE-001`. G-27 queda cerrada documentalmente para el contrato actual; la decision tecnica/arquitectonica pendiente aplica solo a evolucion 409/`Idempotency-Key`/replay. G-28 queda parcial normativo: G3.6 cerro prenotificacion, archivo y flujo tecnico, pero no homologacion. G-30 y G-31 quedan cerrados tecnicamente. G-32 queda cerrado tecnico UAT. G-29 queda cerrado para UAT controlado.

## Actualizacion 2026-06-12 - plan remanente posterior a G3.6

Completado tecnicamente:

1. G3.5 naming dinamico inbound/outbound.
2. G3.5.1 retiro total del mecanismo OpenBao/HashiCorp Vault activo.
3. G3.5.2 migraciones deshabilitadas por defecto y escaneo residual.
4. G3.6A NachaUpload real hasta `Proc_Transacciones` dry-run mediante Quartz.
5. G3.6B NachaExport y dispatch `Proc_Contrapartidas` dry-run correlacionados por `AchCycleId`.

Plan remanente productivo:

1. Homologacion normativa por camara y actas firmadas.
2. Validacion externa de SOAP, sobre digital, firma y certificados.
3. Demostrar causalidad outbound si el alcance futuro exige NachaExport -> dispatch.
4. CENIT/CUD, backup/restore/rollback y monitoreo productivo.
5. Comite final con aprobaciones humanas.

No se debe reinterpretar G3.6B como causalidad directa ni habilitar `Live`. Productivo permanece **NO-GO**.
