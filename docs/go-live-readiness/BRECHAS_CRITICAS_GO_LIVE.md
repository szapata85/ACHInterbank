...
Estado: cierres tecnicos G3.5-G3.6 registrados; requiere triage y aceptacion humana para go-live.
...
| G-04 | ALTO | Naming externo cerrado tecnicamente para NACHA-M; cierre formal por camara pendiente. | Commit `7c3cbb21`; matriz normativa | Riesgo residual de homologacion externa. | Alto | Firmar/homologar reglas por camara y flujo. | Compliance/Operaciones | No para cierre tecnico | Si | Parcial normativo |
...
| G-11 | MEDIO | Custodia de secretos no incluida en docker-compose principal si aplica al ambiente. | `docker-compose.yml` | El provider puede no estar disponible. | Medio/Alto | Se documento que no se modifica compose principal; usar mecanismo del ambiente o excepcion formal. | Seguridad/Operaciones | PENDIENTE | Si | Parcial - requiere decision humana |
...
| G-15 | MEDIO | Health API/DB y Quartz UAT evidenciados; monitoreo productivo externo incompleto. | G3.6 `TaskExecutionLog`, `/health/live`, `/health/ready` | Monitoreo productivo incompleto. | Medio | Diseñar monitoreo/alertas productivas y dependencias externas. | Tecnologia/Operaciones | PENDIENTE | Si | Parcial |
...
| G-28 | ALTO | Generacion/parseo NACHA-M cerrados tecnicamente; validacion formal campo-a-campo y homologacion externa pendientes. | Commits `7c3cbb21`, `e5721150`; G3.6A/G3.6B | Riesgo residual de rechazo externo. | Alto | Obtener firma/waiver y homologacion por camara. | Compliance/Operaciones/Tecnologia | No para cierre tecnico | Si | Parcial normativo |
...
Con cualquier brecha CRITICA abierta, el estado permanece **NO-GO productivo**. G3.5-G3.6 cierran el alcance tecnico de naming, cleanup, inbound y outbound dry-run, pero no homologacion, actas, SOAP real, movimiento monetario ni controles productivos.

## Actualizacion 2026-06-12 - brechas cerradas y remanentes

| Brecha | Estado | Evidencia | Riesgo residual |
|---|---|---|---|
| Ciclo hardcoded `[1-5]` / default `.1` | Cerrada tecnicamente | `7c3cbb21` | `CycleName` debe contener exactamente un entero positivo |
| Dependencia activa OpenBao/HashiCorp Vault | Cerrada tecnicamente | `ebf7a8a5` | KeyVault se evalua separadamente |
| Migraciones automaticas UAT | Cerrada tecnicamente | `c7a5ad50` | Cualquier migracion futura requiere proceso DBA |
| Inbound NachaUpload -> Quartz -> Proc_Transacciones | Cerrada tecnicamente | `e5721150`, G3.6A 2/2 | Dry-run, sin SOAP real ni movimiento monetario |
| Outbound NachaExport + dispatch Proc_Contrapartidas | Cerrada tecnicamente con observacion | `e5721150`, G3.6B 2/2 | Correlacion por `AchCycleId`; causalidad directa no demostrada |

Permanecen abiertas: homologacion externa, CENIT/CUD, sobre digital/certificados, backup/restore/rollback, UAT bancario formal, actas y aprobaciones humanas. Productivo sigue **NO-GO**.
