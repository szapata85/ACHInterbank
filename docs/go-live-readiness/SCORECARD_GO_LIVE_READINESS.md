...
|---|---:|---|---:|---:|---|---|
| Funcionalidad core | 20% | PARCIAL | 78 | 15.6 | Backend CI OK; Angular CI OK; API/DB health OK; proxy SPA->API OK | Falta UAT E2E formal con datos anonimizados |
...
| Operacion y soporte | 10% | PARCIAL | 66 | 6.6 | Docker compose config/build/runtime y proxy SPA->API OK; runbooks/docs | Backup/restore/rollback pendientes |
...
Resultado preliminar tras backend CI OK, angular CI OK, validacion Docker runtime y proxy SPA->API OK: **57.3 / 100 - NO-GO productivo**.
...
- `environment.prod.ts` corregido a base relativa y Nginx proxya `/api`, `/health`, `/openapi` y `/scalar`; falta UAT tecnico funcional con datos anonimizados.
...
- Docker compose config/build/runtime OK para API/PostgreSQL/SPA estatica y proxy SPA->API.
