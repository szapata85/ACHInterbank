...
- Nginx proxya `/auth/`, `/api/`, `/health/`, `/openapi/` y `/scalar` hacia `achinterbank-api:8080`.
...
| SPA Nginx | `web/ach-interbank-ui/nginx.conf` | Sirve SPA y contiene proxy Auth/API/health/OpenAPI/Scalar. |
...
|---|---|---|---|
| SPA no enrutaba API/Auth en compose actual | CERRADO TECNICAMENTE para UAT basico | `nginx.conf` ahora proxya rutas API/Auth; `/api/ach/responses` y `/auth/login` llegan a API. | Ejecutar UAT tecnico con datos anonimizados y usuarios/roles. |
...
El compose actual es **apto para validacion tecnica directa de API, PostgreSQL, OpenAPI/Scalar, SPA estatica y proxy SPA->API/Auth**.
