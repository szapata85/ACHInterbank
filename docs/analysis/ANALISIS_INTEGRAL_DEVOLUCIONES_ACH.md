...
> **JOB 3.1 (cierre provider-specific de B2): CERRADA.** SQL Server y PostgreSQL ejecutaron la misma carrera real con dos `DbContext` independientes que habían observado la ausencia de la identidad; ambos aplicaron sus migraciones y el índice único real. En cada proveedor hubo una aplicación, un replay idempotente, un único evento funcional y causal/estado correctos.
...
| B2 | Idempotencia no DB-first por registro/huérfana/multiarchivo/multinodo. | CRÍTICA | Idempotencia | JOB 3.1; runtime SQL Server/PostgreSQL, `AchTransactionStateEvent.IdempotencyKey` + índice único | doble afectación | **CERRADA**: identidad funcional persistida, atomicidad, carrera provider-specific y replay controlado. |
| B3 | Lifecycle saliente ACH Colombia aplica transición auditada y evidencia en una transacción local; queda evidencia provider-specific/multinodo y CENIT físico bloqueado por homologación. | CRÍTICA | Funcional | JOB4 | cierre parcial, sin éxito inconsistente | Completar evidencia provider-specific y homologación CENIT. |
